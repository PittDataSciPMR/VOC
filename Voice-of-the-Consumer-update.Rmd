...
#Distribution of those who didnt select a DISABILITY TYPER
dizz_na%>%
  group_by(dpct)%>%
  summarize(n=n())%>%
  kable()
...
# Demographic characteristics in those with single conditions
...
```
... Collapse certain groups so that they can be tested reliably  
...
q2%>%
  ...
  group_by(TransportModes)%>%
  mutate(percent=(n()/nrow(q2))*100)%>%
  ggplot(aes(x=TransportModes,label=percent))+
  geom_bar(stat = "count")+
...
q2%>%
  group_by(disab_type,TransportModes)%>%
  summarize(N=n(),
            'n (%)'=paste0
            (
...
#q10
q10<-dataf%>%
  gather(Custom_routes,
         Custom_routesCheck,
         custom_rtes___1:custom_rtes___5,
         factor_key = T)
```
